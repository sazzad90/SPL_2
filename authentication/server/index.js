...
//checking email 
app.post('/forgetPasswordCheckEmail', (req, res) => {
  const email = req.body.email;
...
app.post('/forgetPasswordUpdate', (req, res) => {
  const email = req.body.email;
  const newPassword = req.body.newPassword;

  const password = generateHashedPassword(newPassword);
  db.query("UPDATE userdb SET Password = ? WHERE Email = ?", [password, email], (err, result) => {
    if (err) {
      console.error(err);
    } else {
      res.send(result);
...
//         startingDate = result1[0].starting_date;
//         console.log('starting: ' + startingDate)
//   db.query("SELECT * FROM teams WHERE event_name = ? && year = ?", [eventName, year], (err, result) => {
//     if (err) {
//       console.log(err);
//       res.status(500).send('Error retrieving teams data.');
//     }else{
...
//         console.log('dep_id: ',row.dep_id);

//         db.query("SELECT * FROM department WHERE dep_id = ?", [row.dep_id], (err2, result2) => {
//           if (err2) {
//             reject(err2);
//           } else {
//             data1.push(result2[0].dep_code);
//           }
//         })
//         function generateGroupStageFixture(eventName,numTeams, teams, numGroups, startDate, offDays) {
//                         let teamsPerGroup = Math.floor(numTeams / numGroups);
...
//           const { homeTeam, awayTeam, date, time } = row;
...
app.get('/publishFixture', (req, res) => {
  if (isPublish) {
    db.query('SELECT * FROM events where name = ?', ['Football'], (err, result) => {
      if (err) throw err;
      else {
        console.log('football')
        console.log(result);
        db.query('SELECT * FROM fixture where event_id = ?', [result[0].event_id], (err, results) => {
          if (err) throw err;
          console.log('haha')
          console.log(results);
          res.json(results);
...
app.get('/publishCricketFixture', (req, res) => {
  if (isPublish) {
    db.query('SELECT * FROM events where name = ?', ['Cricket'], (err, result) => {
      if (err) throw err;
      else {
        console.log('cricket')
        console.log(result);
...
app.get('/publishBasketballFixture', (req, res) => {
  if (isPublish) {
    db.query('SELECT * FROM events where name = ?', ['Basketball'], (err, result) => {
      if (err) throw err;
      else {
...
app.get('/publishVolleyballFixture', (req, res) => {
  if (isPublish) {
    db.query('SELECT * FROM events where name = ?', ['Volleyball'], (err, result) => {
      if (err) throw err;
      else {
