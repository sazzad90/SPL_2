...
const ExecutiveNotification =()=> {
    ...
    useEffect(() => {
      const fetchNotifications = async () => {
        const response = await Axios.get('http://localhost:5050/exenotifications');
    ...
    <ul style={{ listStyle: 'none',width: '100%', margin: 0, padding: 0  }}>
       {notifications.map((notification) => (
        <li key={notification.id} style={{textDecoration: 'none'}}>
          <Link to={`http://localhost:5050/executiveNotification/${notification.id}`} style={{ textDecoration: 'none' }}>
          <div class="d-grid">
